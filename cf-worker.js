...
async function handleApi(request, env){
  ...
  try{
    ...
    if(op==="get"){
      const g=await ghGet(c); const r=resolveTask(g.data,payload);
    ...
    if(op==="add"){
      ...
      const r0=resolveTask(data,payload);
      if(r0.error) return json({error:r0.error, matches:r0.matches}, r0.status);
      const t=r0.task;
